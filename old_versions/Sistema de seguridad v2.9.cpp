...
const uint32_t T_backlight=20000;
const uint32_t T_sleep=40000;
...
void sleep()
{
  if(button_hold(RESET_BUTTON_PIN))
  {
    beep_once();
    set_clock=1;
  }
...
//Buttons
uint16_t i_button=0;
const uint16_t I_button=1000;

bool button_hold(int BTN_PIN)
{
    while((digitalRead(BTN_PIN)==1)&&(i_button<I_button))
  {
    i_button++;
  } 
    if(i_button==I_button)
    {
    i_button=0;
    return 1;
    }
...
void Display_Control_s2()
{
  ...
  case 1: //Main DEFCON status monitor
    ...
    myGLCD.print("00:00:00", CENTER, 20);
