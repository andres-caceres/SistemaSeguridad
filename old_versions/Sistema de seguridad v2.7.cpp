...
void Display_Control_s2()
{
  ...
  case 2: //Alert symbol
	...
	myGLCD.update();
	if(millis()-t0_s1>=T_display) //time to screen change
	{
	  Display_State=1;
	}
	break;
...
void energy_saving()
{
  if((millis()-t0_s1>20000)&&(DEFCON==5||DEFCON==0))
  {
    digitalWrite(NOKIA_LED_PIN,LOW); //Backlight off
    
    if(millis()-t0_s1>30000)
    {
      myGLCD.enableSleep();
    }
  }
...
void loop() {
  ...
  t0_s1=millis(); //reset program counter
